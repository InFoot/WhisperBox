...
class FirebaseService {
    ...
    func getUserLetters(userNickname: String, date: Date) async -> Result<[LetterInfo], WhisperBoxError> {
        ...
        let dataSnapshot = await database.child("letters").child(userNickname).child(keyDateString).observeSingleEventAndPreviousSiblingKey(of: .value).0
        for child in dataSnapshot.children {
...
struct GetUserResModel: Hashable {
    var nickname: String
    var password: String
}
